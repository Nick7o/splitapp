...
const BottomNav: React.FC = () => {
  ...
  const navButtonClass = (active: boolean) =>
    `flex h-14 min-w-[56px] flex-1 flex-col items-center justify-center gap-0.5 rounded-xl px-2 transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-primary-fixed/50 active:scale-95 md:h-14 md:flex-none md:w-full md:px-2 ${
      active
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-50 px-2 pb-[calc(env(safe-area-inset-bottom)+0.5rem)] md:inset-y-0 md:left-0 md:right-auto md:w-24 md:px-0 md:pb-0">
      <div className="mx-auto flex w-full max-w-md items-center justify-around gap-1 rounded-2xl border border-white/10 bg-surface/88 p-1.5 shadow-[0_-10px_34px_rgba(2,6,23,0.32)] backdrop-blur-2xl md:h-full md:max-w-none md:flex-col md:justify-center md:gap-3 md:rounded-none md:border-y-0 md:border-l-0 md:border-r md:px-3 md:py-6 md:shadow-[10px_0_32px_rgba(2,6,23,0.22)]">
        <button 
        ...
        >
          ...
          <span className="text-[10px] font-bold leading-none">{t('nav.groups')}</span>
        </button>
        ...
        <button 
          ...
          className="flex h-14 min-w-[56px] flex-1 flex-col items-center justify-center gap-0.5 rounded-xl px-2 text-secondary transition-all duration-150 hover:bg-white/5 hover:text-on-surface focus:outline-none focus:ring-2 focus:ring-primary-fixed/50 active:scale-95 md:h-14 md:w-full md:flex-none md:px-2"
        >
          <span className="material-symbols-outlined text-2xl drop-shadow-sm" style={{ fontVariationSettings: "'FILL' 1" }}>
            {shouldAddExpense ? 'receipt_long' : 'group_add'}
          </span>
          <span className="text-[10px] font-bold leading-none">{shouldAddExpense ? t('nav.addExpenseShort') : t('nav.newGroupShort')}</span>
        </button>
        ...
        >
          ...
          <span className="text-[10px] font-bold leading-none">{t('nav.activity')}</span>
        </button>
        ...
        >
          ...
          <span className="text-[10px] font-bold leading-none">{t('nav.profile')}</span>
        </button>
      </div>
    </nav>
  );
