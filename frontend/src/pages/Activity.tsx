      ...
      <div className="space-y-6">
        <section className="app-card p-4 sm:p-5">
          ...
          {!loading && items.length > 0 ? (
            <p className="mt-3 text-xs font-semibold text-on-surface-variant">
              {t('activity.resultSummary', { shown: filteredItems.length, total: items.length })}
            </p>
          ) : null}
        </section>
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-dashed border-white/15 bg-surface-container-lowest/70 px-5 py-16 text-center text-on-surface-variant">
            <span className="material-symbols-outlined mb-4 text-6xl opacity-50">notifications_off</span>
            <p className="font-headline text-xl text-on-surface">{t('activity.emptyTitle')}</p>
